...
//            }
//            if (Objects.equals(x.specialScholarship(), x.fullName() + " have special scholarship privilege"))
//                allSc.add(x);
//        }
//
...
//                System.out.println(x);
//            if (Objects.equals(x.specialScholarshipStudent(), x.fullName() + " have special scholarship privilege"))
//                allSc.add(x);
//        }
//        System.out.println();
//        for (Human x: allSc)
//            System.out.println(x);
//    }
    private static void itScoreDouble(double score) throws CustomException{
        if (score < 2 || score > 5){
            throw new CustomException("The score must be in a range from 2 to 5");
        }
    }
    private static void itScoreInt(int score) throws CustomException{
        if (score < 2 || score > 5){
            throw new CustomException("The score must be in a range from 2 to 5");
        }
    }
    ...
    public static void arrayStudent(ArrayList<Integer> gradesStudent, ArrayList<Integer> courseworkStudent){
        ...
        while (amountOfSub != 0){
            ...
            if (sc.hasNextInt()) {
                try {
                    itScoreInt(Integer.parseInt(sc.next()));
                }
                catch (CustomException e){
                    System.out.println("Error: " + e.getMessage());
                    continue;
